            ...
            foreach (string continent in continentsNames)
            {
                ...
                if (entry.getAdditionalData().TryGetValue(continent, out casesString))
                {
                    ...
                    if (dataMap.TryGetValue(continent, out data)){
                        ...
                        data.graph.transform.position = this.gameObject.transform.position + new Vector3(4 + 1.5f * scale + 2.5f * System.Array.IndexOf(continentsNames, continent), -scale, 0);
                    } else {
                        // Create bar
                        ...
                        // Set graph location
                        data.graph.transform.position = this.gameObject.transform.position + new Vector3(3.5f + 1.5f * scale + 2.5f * System.Array.IndexOf(continentsNames, continent), -scale, 0);
                        // Add text
